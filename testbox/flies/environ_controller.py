...
from nav_msgs.msg import Odometry ### THE SOLUTION
...
class GameController:
    ...
    def main(self, args=None):
        ...
        This is where the ball is projected and the next position is chosen and then sent to the correspending flyer.
        """
            ...
            while True:
                ...
                if isinstance(frame, np.ndarray) and frame.any():
                    ...
                    if np.all(ids is not None) or not self.NEED_CALIBRATION:
                        ...
                        # Draw the ball
                        cv.circle(overlay_ballon,
                                  (int(p_image_normalized_ballon[0]), int(p_image_normalized_ballon[1])),
                        ...
                        if len(trajectory_positions) > 1:
                            ...
                            flyer = self.get_next_flyer()

                            end_pos = (
                                round(random.uniform(flyer.min_x, flyer.max_x), 2),
                                round(random.uniform(flyer.min_y, flyer.max_y), 2),
                            ...
                            flyer.position_to_visit = position_to_visit
